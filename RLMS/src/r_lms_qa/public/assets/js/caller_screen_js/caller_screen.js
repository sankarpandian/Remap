$("document").ready(function()
{
	 ...
	 $(document).on("change","#lld_CallFromId",function()
	{
		//$("div#isp_info_container").hide();
		//$("div#customer_present_container").hide();

		var call_from_id 	= $.trim( $("#lld_CallFromId").val() ); 
		
		
		if( $("#lld_CallFromId").is("select") )
		{
			//case - element is 'select' 
			var call_from 		= $.trim( $("#lld_CallFromId option:selected").attr("code") ); //$("#list option:selected").text();
		}
		else
		{
			//case - element is 'input' 
			var call_from 		= $.trim( $("#lld_CallFromId").attr("code") ); //$("#list option:selected").text();
		}
			...
			$.ajax({
			  ...
			  //  getresult += '<select class="form-control" name="dnis_info" id="dnis_info" >';
				...
				//getresult += '</select>';
				   ...
				   $("select#dnis_info").html(getresult).show();
				   $("#lld_CallFromId").attr("disabled", false);
				  // $("#dnis_info").attr('class','form-control');
				   //auto seleect description for SMart circle
				   if($("#lld_CallFromId").val() == '2')
				   {
				   	$("#dnis_info").val('DSA');
				   }
			   }
	...
	$('#lld_AssociateId').focus(function() 
	{
		$(this).select();
	});
		//Script to change HD customer / associate
	$(document).on("change","#lld_CallTypeId",function(){
		
		$("div#customer_present_container").hide();
		if( parseInt( $("#lld_CallFromId").val() ) == 2 )
		{
...
$("select#dnis_info").on("change", function(){
	//alert('hihi');
		...
		//alert(desc_val);
		var call_from	= $.trim( $("select#lld_CallFromId").val() );
