...
function leaveAStepCallback(obj, context){
    ...
    function validateAllSteps()
	{
		...
		WPhoneTitle               = $.trim( $("#lcd_WPTitle").val() ); 
		CPhoneTitle               = $.trim( $("#lcd_CPTitle").val() ); 
...
var appt_date      = $(this).attr("data-date-only");
var time_get       = $(this).attr("data-time");
var month_name_get       = $(this).attr("data-month-name");
$("#get_time_dis").val(time_get);
$("#get_date_dis").val(appt_date);
$("#appointment_date_time").val(get_date_time+', '+month_name_get+' '+date_get+'@'+time_get);
$("#display_scheduled_time").html(get_date_time+', '+month_name_get+' '+date_get+'@'+time_get);
...
var zipcode = $("#zipcode").val();
			...
			$.ajax({
			   ...
			   success: function(data){
				  // alert(data);
				  // $("span.product_list_container").html("");
			$("#lld_ProductCode").html("");
			$("#lcd_City").html("");
			$("#lcd_County").html("");
			$("#lcd_State").html(""); 
		...
		if ($("#oot").prop('checked') == true) 
		{
        	...
        	if (box==true)
        	{
				...
				success: function(data){
					if(data=="success")
					{
						var url = 'Rescheduleoutboundcallerscreen';
						window.location.href = url;
					}
...
<?php 
  $media_arr    = str_split(trim($RescheduleCustdetails['lld_ProsepctId']), 3); //personsecondphone
  $media    = $media_arr[0];
...
<input type="hidden" id="lld_CustomerId" name="lld_CustomerId" value="{{$RescheduleCustdetails['lld_CustomerId']}}">	 
...
<input type="hidden" id="requestId" name="requestId" value="{{$requestId}}">         
	...
	<div class="x_content">
	<input type="hidden" name="prospect_id" id="prospect_id" value="@if(isset($RescheduleCustdetails['lld_ProsepctId'])) {{$RescheduleCustdetails['lld_ProsepctId']}} @endIf"  />
	<p>Thank you for calling The Home Depot Installed Services. My name is Christudasan.</p>
	  <div class="form-group">
		...
		<select class="form-control" name="call_from" id="call_from" data-parsley-group="block1" required>
			...
			<option value="{{$CallerScreenRepository_data[$i]['lcf_CallFromId']}}" code="{{$CallerScreenRepository_data[$i]['lcf_CallFromCode']}}" @if($RescheduleCustdetails['lld_CallFromId']==$CallerScreenRepository_data[$i]['lcf_CallFromId']){{"selected"}} @endIf>{{$CallerScreenRepository_data[$i]['lcf_CallFromName']}}
			</option>
			@endfor
		</select>
		</div>
      ...
      <div class="form-group">
		...
		<select class="form-control" id="title" name="title" data-parsley-group="block2" required>
			<option value="">Select One</option>
			<option value="mr" @if($RescheduleCustdetails['lcd_Title']=='mr') {{"selected"}} @else {{" "}} @endIf>Mr.</option>
			<option value="mrs" @if($RescheduleCustdetails['lcd_Title']=='mrs') {{"selected"}} @else {{" "}} @endIf>Mrs.</option>
	    </select>
		</div>
		<div class="col-md-3">                         
		<input type="text" id="first_name" name="usrs_firstname" required class="form-control col-md-7 col-xs-12" value="{{$RescheduleCustdetails['lcd_FirstName']}}" data-parsley-group="block2">
		</div>
		<div class="col-md-3">                         
		<input type="text" id="last_name" name="usrs_lastname" required class="form-control col-md-7 col-xs-12" value="{{$RescheduleCustdetails['lcd_LastName']}}" data-parsley-group="block2">
		</div>
	 </div>	 
	...
	<div class="form-group">
		<label class="control-label col-md-12" for="first-name"> I apologize but due to a scheduling conflict we will not be able to meet with you July 01 at 11:00 AM as scheduled.  
		</label>	
	</div>
	
	<div class="form-group">
		<label class="control-label col-md-12" for="first-name"> What I'd like to do is go ahead and reschedule a convenient time to show you our Cabinet Refacing. 
 		</label>	
	</div>
	
	 <div class="form-group">
		<label class="control-label col-md-2" for="first-name"> I have your zip code as</label>
		<div class="col-md-3">                         
		<input type="text" id="zipcode" name="zipcode" required class="form-control" value="@if(isset($RescheduleCustdetails['lcd_Zipcode'])){{$RescheduleCustdetails['lcd_Zipcode']}}@else{{" "}}@endIf" data-parsley-group="block2" data-parsley-type="digits" data-parsley-length="[5, 5]" data-parsley-length-message ="Please fill 5 digit Zipcode">
		</div><label class="control-label col-md-4" for="first-name">and just to reconfirm you are interested in seeing our</label>
		<div class="col-md-2">                         
			<select name="lld_ProductCode" id="lld_ProductCode" class="form-control" data-parsley-group="block2" required>
				<option value="">Select One</option>
				<option value="K" selected>Cabinet Refacing</option>
				<option value="G">Garage Organization Systems</option>
				<option value="O">Closet Organization Systems</option>
			</select>
		</div>
	 ...
	 <div class="form-group">
     <label class="control-label col-md-12" for="first-name">Ok great.</label>	
	</div>
	</div> 		
  </div> 
 </div> 
  ...
  <div class="col-md-12 col-sm-12 col-xs-12">
	...
	<div class="clearfix"></div>
	  ...
	  <div class="x_content">
		 ...
		 <div class="form-group">
			<label class="control-label col-md-12" for="first-name"> <input type="checkbox" name="product_mode" id="nonproduct" value="11" > Non Product 
		...
		<div class="form-group">
		<label class="control-label col-md-12" for="first-name"> Mr./Mrs. JESSICA JOYNER, 
		</label>	
	    </div>	
		
		<div class="form-group">
		<label class="control-label col-md-12" for="first-name"> (If cabinets are new – watch for countertops only and service related issues)
		</label>	
	    </div>
		
		<div class="form-group">
		<label class="control-label col-md-12" for="first-name"> Our Design Consultant will be showing you cabinet refacing and not new cabinets, correct? Cabinet refacing is removing your existing doors, drawers and drawer fronts and throwing them away. We custom build new doors, drawers and drawer fronts and cover all exposed surfaces with a matching veneer or laminate. Is this what you were interested in seeing?   
		</label>	
	    </div>
		
		<div class="form-group">
		<label class="control-label col-md-12" for="first-name"> In addition to cabinet refacing what other improvements are you considering for your kitchen? (If countertops, verify not just countertops only) Great, Mr./Mrs. Jeron James I will make a note of that for our consultant. 
		</label>	
	    </div>
...
<div class="form-group">
	...
	<div class="col-md-9 col-sm-9 col-xs-12">                         
		<input type="text" class="form-control" name="customer_address" id="customer_address" required placeholder="Address" value="@if(isset($RescheduleCustdetails['lcd_Address'])){{$RescheduleCustdetails['lcd_Address']}} @else {{""}} @endIf" data-parsley-group="block5">
	</div>
</div>
...
<div class="form-group">
	...
	<div class="col-md-9 col-sm-9 col-xs-12">                         
		<select class="form-control" name="lcd_State" id="lcd_State" required data-parsley-group="block5">
...
<div class="form-group">
	...
	<div class="col-md-9 col-sm-9 col-xs-12">                         
		<input type="text" class="form-control" name="customer_cross_street" id="customer_cross_street" placeholder="Cross Street" required value="@if(isset($customerDetails['RescheduleCustdetails'])){{$customerDetails['RescheduleCustdetails']}} @else {{""}} @endIf" data-parsley-group="block5"> correct? Ok, Great
	</div>
</div>
...
<div class="form-group">
	...
	<div class="col-md-9 col-sm-9 col-xs-12">                         
		<input type="text" class="form-control" name="customer_community" id="customer_community" required placeholder="Community Name" value="@if(isset($RescheduleCustdetails['lcd_Community'])){{$RescheduleCustdetails['lcd_Community']}} @else {{""}} @endIf" data-parsley-group="block5">
	</div>
</div>	
...
<div class="form-group">
	...
	<div class="col-md-9 col-sm-9 col-xs-12">                         
		<input type="text" class="form-control" name="house_color" id="house_color" required placeholder="House Color" value="@if(isset($RescheduleCustdetails['lcd_HousecColor'])){{$RescheduleCustdetails['lcd_HousecColor']}} @else {{""}} @endIf" data-parsley-group="block5">
	</div>
</div>
...
<div class="form-group">
	...
	<div class="col-md-9 col-sm-9 col-xs-12">                         
		<input type="text" class="form-control" name="home_phone" id="home_phone" required placeholder="Home Phone" value="@if(isset($RescheduleCustdetails['lcd_HomePhone'])){{$RescheduleCustdetails['lcd_HomePhone']}} @else {{""}} @endIf" data-parsley-group="block5">
	</div>
</div>
...
<div class="form-group">
	...
	<div class="col-md-9 col-sm-9 col-xs-12">                         
		<input type="text" class="form-control" name="work_phone" id="work_phone" placeholder="Work Phone" value="@if(isset($RescheduleCustdetails['lcd_WorkPhone'])){{$RescheduleCustdetails['lcd_WorkPhone']}} @else {{""}} @endIf" data-parsley-group="block5">
	</div>
</div>
...
<div class="form-group">
	...
	<div class="col-md-9 col-sm-9 col-xs-12">                         
		<select name="lcd_WPTitle" id="lcd_WPTitle" class="form-control col-md-10" style="float: left;" required data-parsley-group="block5">
			<option value="">Select One</option>
			<option value="mr" @if($RescheduleCustdetails['lcd_WPTitle']="mr"){{"selected"}} @else {{""}} @endIf>Mr.</option>
			<option value="mrs" @if($RescheduleCustdetails['lcd_WPTitle']="mrs"){{"selected"}} @else {{""}} @endIf>Mrs.</option>
		</select>
	</div>
</div>
...
<div class="form-group">
	...
	<div class="col-md-9 col-sm-9 col-xs-12">                         
		<input type="text" class="form-control" name="cell_phone" id="cell_phone" placeholder="Cell Phone" value="@if(isset($RescheduleCustdetails['lcd_CellPhone'])){{$RescheduleCustdetails['lcd_CellPhone']}} @else {{""}} @endIf" data-parsley-group="block5">
	</div>
</div>
...
<div class="form-group">
	...
	<div class="col-md-9 col-sm-9 col-xs-12">                         
		<select name="lcd_CPTitle" id="	" class="form-control col-md-10" style="float: left;" required data-parsley-group="block5">
			<option value="">Select One</option>
			<option value="mr" @if($RescheduleCustdetails['lcd_CPTitle']="mr"){{"selected"}} @else {{""}} @endIf>Mr.</option>
			<option value="mrs" @if($RescheduleCustdetails['lcd_CPTitle']="mrs"){{"selected"}} @else {{""}} @endIf>Mrs.</option>
		</select>
	</div>
</div>
...
<div class="form-group">
	...
	<div class="col-md-9 col-sm-9 col-xs-12">                         
		<input type="text" class="form-control" name="customer_comments" id="customer_comments" placeholder="Additional Comments" value="@if(isset($RescheduleCustdetails['lcd_Comments'])){{$RescheduleCustdetails['lcd_Comments']}} @else {{""}} @endIf" data-parsley-group="block5"> 
	</div>
</div>
		...
		<div class="x_content">
		  <div class="form-group" style="margin:40px 30px;">
		    <label class="control-label col-md-9 col-sm-3 col-xs-12" for="first-name"> 
             Let me confirm all of your information.
			</label>
			<label class="control-label col-md-9 col-sm-3 col-xs-12" for="first-name"> 
              Mr TEST,
			</label>
