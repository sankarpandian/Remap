	          ...
	          if(lm_FieldType=='select')
				{
					...
					if(lm_FieldID=="title")
					{
						...
						var AllInfo = AllInfo+"<div class='col-md-5 col-sm-5 col-xs-12'><select class='form-control' name='"+lm_FieldName+"' id='"+lm_FieldID+"'><option value=''>Select</option><option value='Mr'>Mr</option><option value='Mrs'>Mrs</option></select></div>";
						$(".title").html(AllInfo);
					}
				...
				else if(lm_FieldType=='text' || lm_FieldType=='hidden' || lm_FieldType=='checkbox' || lm_FieldType=='radio')
				{
					...
					var callList = '<input type="'+lm_FieldType+'"  name="'+lm_FieldName+'" class="'+lm_FieldClass+'" id="'+lm_FieldID+'">';
...
function AllSlideListInbound(typeMeth,dataType,urlPass,dataReq,SreenName)
{
	$.ajax({type:typeMeth,dataType:dataType,data:dataReq,url:urlPass,success:function(data){
        ...
        for(var i=0;i<data.length;i++)
        {
           ...
           /* if(callFrom == lm_FieldID)
            {
				//alert(callFrom);
               var callFromResult = CreateFieldInfoFunc(lm_FieldID,lm_FieldClass,lm_FieldName,lm_FieldType);
			   //$(".callFrom").html(callFromResult);
			}
			if(dnis_info == lm_FieldID)
            {
               var dnis_infoResult = CreateFieldInfoFunc(lm_FieldID,lm_FieldClass,lm_FieldName,lm_FieldType);
			  // $(".dnis_info").html(dnis_infoResult);
			}
			if(hd_type_id == hd_type_id)
            {
               var dnis_infoResult = CreateFieldInfoFunc(lm_FieldID,lm_FieldClass,lm_FieldName,lm_FieldType);
			   //alert(dnis_infoResult);
...
// start on dynamic script js
    ...
    {
        var callFrom     = $(".call_from").attr('class');
		//alert(callFrom);
    }
	
    if(($(".dnis_info").length)>0)
    {
        var dnis_info = $(".dnis_info").attr('class');
    }
    if(($(".hd_type_id").length)>0)
    {
        var hd_type_id  = $(".hd_type_id").attr('class');
    }
     if(($(".hd_type_id").length)>0)
    {
        var hd_type_id  = $(".hd_type_id").attr('class');
	...
	if(($(".title").length)>0)
    {
        var title  = $(".title").attr('class');
    }
	if(($(".first_name").length)>0)
    {
        var first_name  = $(".first_name").attr('class');
    }
	if(($(".last_name").length)>0)
    {
        var last_name  = $(".last_name").attr('class');
    }
	if(($(".zipcode").length)>0)
    {
        var zipcode  = $(".zipcode").attr('class');
    }
    // Greetings
	var data = {screenId:'2'};
