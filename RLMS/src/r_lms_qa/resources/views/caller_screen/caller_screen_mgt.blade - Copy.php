...
$("input[name='non_product_status']").change(function(){
	...
	if ($("#nonproduct").prop('checked') == true) 
		...
		else if($("#nonproduct").prop('checked') == false) 
		{
			//alert("Please select non product checkbox");
...
function ajaxcallSelectData(lm_FieldID,GET,Json,dataList,url,classAttr,nameAttr)
{
	var callList = "";
	callList = callList+'<div class="col-md-5 col-sm-5 col-xs-12">';
					$.ajax({type:GET,dataType:Json,data:dataList,url:url
						 ,success:function(result)
						 {
							// alert(lm_FieldID);
							if(lm_FieldID=='call_from')
							{
								callList = callList+'<select name="'+nameAttr+'" class="'+classAttr+'" id="'+lm_FieldID+'" data-parsley-group="block1" required=""><option value="">Select</option>';
								for(var i=0;i<result.length;i++)
								{
									callList = callList+'<option value="'+result[i].lcf_CallFromId+'" code="'+result[i].lcf_CallFromCode+'">'+result[i].lcf_CallFromName+'</option>';
								}
								callList = callList+'</select></div>'; 
							    $(".call_from").html(callList);
							}
							if(lm_FieldID=='hd_type_id')
							{
								
								callList = callList+'<select name="'+nameAttr+'" class="'+classAttr+'" id="'+lm_FieldID+'" data-parsley-group="block1" required=""><option value="">Select</option>';
								for(var j=0;j<result.length;j++)
								{
									//alert(result[j]['lht_HdTypeId']);
									callList = callList+'<option value="'+result[j].lht_HdTypeId+'" >'+result[j].lht_HdType+'</option>';
								}
								callList = callList+'</select></div>'; 
								//alert(callList);
								$(".hd_type_id").html(callList);
							}
							if(lm_FieldID=='lld_CallFromId')
							{
								
								callList = callList+'<select name="'+nameAttr+'" class="'+classAttr+'" id="'+lm_FieldID+'" data-parsley-group="block4" required=""><option value="">Select</option>';
								
								for(var j1=0;j1<result.length;j1++)
								{
									callList = callList+'<option value="'+result[j1].lch_HomeTypeId+'" >'+result[j1].lch_HomeTypeName+'</option>';
								}
								callList = callList+'</select></div>'; 
								$(".lld_CallFromId").html(callList);
							}
...
function CreateFieldInfoFunc(lm_FieldID,lm_FieldClass,lm_FieldName,lm_FieldType,SreenName)
{ 
	          var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
			  var AllInfo = "";
	          ...
	          if(lm_FieldType=='select')
				{
					...
					{
					// alert(lm_FieldID);	
					data = {callFromData:"callFromData",DataFieldID:lm_FieldID,_token: CSRF_TOKEN};
					// ajax function call for List of all data for call from
					ajaxcallSelectData('call_from','GET','json',data,'./ListOfAllDataSelect',lm_FieldClass,lm_FieldName); 
                    }
					if(lm_FieldID=="hd_type_id")
					{
						AllInfo = AllInfo+"<div class='col-md-5 col-sm-5 col-xs-12'><select class='form-control' name='"+lm_FieldName+"' id='"+lm_FieldID+"'><option value=''>Select</option></select></div>";
						//$(".hd_type_id").html(dnisInfo);
						data = {callFromData:"hdTypeData",DataFieldID:lm_FieldID,_token: CSRF_TOKEN};
						// call ajax function
						ajaxcallSelectData('hd_type_id','GET','json',data,'./ListOfAllDataSelect',lm_FieldClass,lm_FieldName); 
						
					}
					// Customer info
					if(lm_FieldID=="lld_CallFromId")
					{
						AllInfo = AllInfo+"<div class='col-md-9 col-sm-5 col-xs-12'><select class='form-control' name='"+lm_FieldName+"' id='"+lm_FieldID+"'><option value=''>Select</option></select></div>";
						//$(".hd_type_id").html(dnisInfo);
						data = {callFromData:"CustFamilyType",DataFieldID:lm_FieldID,_token: CSRF_TOKEN};
						// call ajax function
						ajaxcallSelectData('lld_CallFromId','GET','json',data,'./ListOfAllDataSelect',lm_FieldClass,lm_FieldName); 
						
					}
					if(lm_FieldID=="dnis_info")
					{
						AllInfo = AllInfo+"<div class='col-md-5 col-sm-5 col-xs-12'><select class='"+lm_FieldClass+"' name='"+lm_FieldName+"' id='"+lm_FieldID+"'><option value=''>Select</option></select></div>";
						$(".dnis_info").html(AllInfo);
					}
					
					if(lm_FieldID=="title")
					{
						
						var AllInfo = AllInfo+"<div class='col-md-5 col-sm-5 col-xs-12'><select class='form-control' name='"+lm_FieldName+"' id='"+lm_FieldID+"' data-parsley-group='block2' required=''><option value=''>Select</option><option value='Mr'>Mr</option><option value='Mrs'>Mrs</option></select></div>";
						$(".title").html(AllInfo);
					}
					if(lm_FieldID=="lcd_State")
					{
						
						var AllInfo = AllInfo+"<div class='col-md-9 col-sm-5 col-xs-12'><select class='form-control' name='"+lm_FieldName+"' id='"+lm_FieldID+"'><option value=''>Select</option></select></div>";
						$(".lcd_State").html(AllInfo);
					}
					if(lm_FieldID=="lcd_City")
					{
						
						var AllInfo = AllInfo+"<div class='col-md-9 col-sm-5 col-xs-12'><select class='form-control' name='"+lm_FieldName+"' id='"+lm_FieldID+"'><option value=''>Select</option></select></div>";
						$(".lcd_City").html(AllInfo);
					}
					if(lm_FieldID=="lcd_WPTitle")
					{
						
						var AllInfo = AllInfo+"<div class='col-md-9 col-sm-5 col-xs-12'><select class='form-control' name='"+lm_FieldName+"' id='"+lm_FieldID+"'><option value=''>Select</option><option value='Mr'>Mr</option><option value='Mrs'>Mrs</option></select></div>";
						$(".lcd_WPTitle").html(AllInfo);
					}
					if(lm_FieldID=="lcd_CPTitle")
					{
						
						var AllInfo = AllInfo+"<div class='col-md-9 col-sm-5 col-xs-12'><select class='form-control' name='"+lm_FieldName+"' id='"+lm_FieldID+"'><option value=''>Select</option><option value='Mr'>Mr</option><option value='Mrs'>Mrs</option></select></div>";
						$(".lcd_CPTitle").html(AllInfo);
					}
				...
				{
					var callList="";
					callList = callList+'<input type="'+lm_FieldType+'"  name="'+lm_FieldName+'" class="'+lm_FieldClass+'" id="'+lm_FieldID+'"'; 
					
					if(lm_FieldID=="first_name")
					{
						callList = callList+' data-parsley-group="block2" required="">';
					 $(".first_name").html(callList);
					}
					if(lm_FieldID=="last_name")
					{
					callList = callList+' data-parsley-group="block2" required="">';
					 $(".last_name").html(callList);
					}
					if(lm_FieldID=="zipcode")
					{
					  callList = callList+' data-parsley-group="block2" required="">';
					 $(".zipcode").html(callList);
					}
					// customer info 
					if(lm_FieldID=="not_owner")
					{
					 callList = callList+'>';
					 $(".not_owner").html(callList);
					}
					if(lm_FieldID=="customer_address")
					{
					 callList = callList+' data-parsley-group="block4" required="">';	
					 $(".customer_address").html(callList);
					}
					if(lm_FieldID=="customer_cross_street")
					{
					 callList = callList+' data-parsley-group="block4" required="">';	
					 $(".customer_cross_street").html(callList);
					}
					if(lm_FieldID=="zipcode2")
					{
					 callList = callList+' data-parsley-group="block4" required="">';
					 $(".zipcode2").html(callList);
					}
					
					if(lm_FieldID=="customer_community")
					{
					 callList = callList+'>';
					 $(".customer_community").html(callList);
					}
					if(lm_FieldID=="house_color")
					{
					 callList = callList+'>';
					 $(".house_color").html(callList);
					}
					if(lm_FieldID=="home_phone")
					{
					 callList = callList+' data-parsley-group="block4" required="">';	
					 $(".home_phone").html(callList);
					}
					if(lm_FieldID=="work_phone")
					{
					  callList = callList+'>';
					 $(".work_phone").html(callList);
					}
					if(lm_FieldID=="cell_phone")
					{
					  callList = callList+'>';
					 $(".cell_phone").html(callList);
					}
					if(lm_FieldID=="lcd_email")
					{
					  callList = callList+'>';
					 $(".lcd_email").html(callList);
					}
				...
				{
					var callList = '<textarea name="'+lm_FieldName+'" class="'+lm_FieldClass+'" id="'+lm_FieldID+'"></textarea>';
...
{
	$.ajax({type:typeMeth,dataType:dataType,data:dataReq,url:urlPass,success:function(data){
        ...
        for(var i=0;i<data.length;i++)
        {
		    ...
		    CreateFieldInfoFunc(lm_FieldID,lm_FieldClass,lm_FieldName,lm_FieldType,SreenName);
	...
	<div class="clearfix"></div>
	 <div style="float: right;position:absolute;right:20px;top:20px;">
	  ...
	  <div class="x_content">
		 ...
		 <div class="clearfix"></div>
		 <div class="form-group">
			<label class="control-label col-md-2 col-sm-3 col-xs-12" for="first-name"> <input type="checkbox" name="product_mode" id="nonproduct" value="10" data-parsley-group="block3"> Non Product 
			</label>
			<label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">                        
			<input type="checkbox" name="product_mode" id="oot" value="11" data-parsley-group="block3"> OOT / Service Not Available in <span class="zipcode1">30123</span>
			</label>
		 </div>
		 <br>
		 ...
		 <div class="col-md-5 col-sm-5 col-xs-12">                         
		<input type="text" id="phone" name="phone"  class="form-control col-md-7 col-xs-12" data-parsley-group="block3">
		</div>
		</div>
