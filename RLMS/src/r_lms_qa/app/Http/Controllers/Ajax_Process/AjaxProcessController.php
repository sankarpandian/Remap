...
*/
	...
	{
		$call_from 			= $_REQUEST['lld_CallFromId'];	
		...
		$dnis_info       = trim($_REQUEST['call_desc_container']);
	...
	$customer_mode_id   	= trim( $_REQUEST['customer_mode_id'] );; //Not Available
	$spouse_name			= "";
	$customer_address		= "";
	...
	//$customer_county		=  trim( $_REQUEST['lcd_County'] );
	$customer_county		=  1;
	...
	else
	{
		...
		$lms_calldetails->lld_CompanyId = $companyId;
		$lms_calldetails->save();
		
	$actual = $this->CalendarRepository->slotDetailsForAllLeads($appdate,$appTime,$zipcode,$customer_id,$territory_id,$companyId);
  ...
  return json_encode(array("kitchen"=>$kitchen,"bath"=>$bath,"garage"=>$garage,"closet"=>$closet,"city"=>$city,"county"=>$county,"state"=>$state,"territory_code"=>$territory_code,"territory_id"=>$territory_id));
	...
	public function ajax_oot_inbound()
	{
		/*$oot			     = $_REQUEST['oot'];
		$call_from			 = $_REQUEST['call_from'];
		$dnis_info			 = $_REQUEST['dnis_info'];
		$hd_type_id			 = $_REQUEST['hd_type_id'];
		$associate_id		 = $_REQUEST['associate_id'];
		$store_no			 = $_REQUEST['store_no']; 
		$customer_presence	 = $_REQUEST['customer_presence']; 
		...
		$lms_customerdetails->lcd_LastName    = $usrs_lastname;
		$lms_customerdetails->lcd_Zipcode     = $zipcode;
		$lms_customerdetails->lcd_HomePhone     = $phone;	
		$lms_customerdetails->save();
		...
		$lms_calldetails->lld_CallStatusId = $oot;
		
		$lms_calldetails->save();
		return json_encode("success");
	}

	public function ajax_non_product_inbound()
	{
		extract($_REQUEST);
		...
		$lms_calldetails->lld_CallStatusId = $nonproduct;
		$lms_calldetails->lld_NonProductId = $non_product_status;
