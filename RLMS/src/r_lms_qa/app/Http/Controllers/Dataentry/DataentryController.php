...
{ 
   public function __construct(home $home,CalendarRepository $CalendarRepository,CallerScreenRepository $CallerScreenRepository)
	...
	{	
		...
		$message=array('title' => 'Data Entry',  'description' => "" , 'page' => 'dataentry','get_dashborad_data' => $list_dashborad_data);
		//return redirect()->route('Auth.login', [$error_message]);
		$call_froms=lms_callfroms::all();

		return view('dataentry.dataentry',$message)->with(array("call_froms"  => $call_froms));
	...
	{		
		...
		return view('dataentry.dataentry1',$message)->with(array("hometypes"  => $hometypes))
		->with(array("lld_CallFromId"=>$_POST['lld_CallFromId'],"calldescription"=>$calldescription,"lld_AssociateId"=>$_POST['lld_AssociateId'],"lld_StoreId"=>$_POST['lld_StoreId'],"lld_calldescription"=>$_POST['lld_Calldesc']))->with('agent_user_list',$agent_user_list);
	}
	...
	{		
		...
		if((isset($appointment_date) && $appointment_date!='') && (isset($appointment_time) && $appointment_time!=''))
		{
		$actual = $this->CalendarRepository->slotDetailsForAllLeads($appointment_date,$appointment_time,$lcd_Zipcode,$CustomerId,$territory_id);
		...
		return redirect('gridview');
		 ...
		 else 
		 {
			...
			return Redirect::to('dataentry')->withErrors($validator); 
		 }
