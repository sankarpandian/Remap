...
class LeadsImportApiController extends Controller
{
	 ...
	 public function leads_import_api()
	{
		...
		//echo $security_key;
		$xmlString=urldecode($_REQUEST['xml_string']);
		...
		$territoryId = $this->CalendarRepository->get_territory_based_zipcode($leads->zipcode);
		$lms_customerdetails->lcd_Zipcode =$leads->zipcode;
		$lms_customerdetails->lcd_Territory =$territoryId;
		...
		$lms_customerdetails->lcd_CompanyId =$leads->company_id;
		...
		$remap_outbound_leads->rol_CompanyId =$leads->company_id;
		...
		else
		{
			...
			$lms_duplicaterecords->ldr_CompanyId =$leads->company_id;
