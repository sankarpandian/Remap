                  ...
                  <div class="x_content">
                    ...
                    <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                     ...
                     <tbody id="get_week_list">
                        @for($i=0;$i<count($SmDashboardResult);$i++)
                        <tr>
                          <td>{{$i+1}}</td>
                          <td>
						  <form action="salesmanagerweeklyrequest" method="POST">
						  <input type="hidden" name="_token" value="{{ csrf_token() }}">
						  <input type="hidden" name="week_id" value="{{$SmDashboardResult[$i]['lw_WeekId']}}">
						  <input type="hidden" name="slot_id" value="{{$SmDashboardResult[$i]['ls_SlotId']}}">
						  <input type="hidden" name="territory_id" value="{{$SmDashboardResult[$i]['ls_TerritoryId']}}">
						  <input type="hidden" name="calendar_id" value="{{$SmDashboardResult[$i]['ls_CalenProduct']}}">
						  <button type="submit" class="btn btn-link">{{$SmDashboardResult[$i]['lw_WeekStartDate']}} to {{$SmDashboardResult[$i]['lw_WeekEndDate']}}</button>
						  </form>
						  
						  </td>
                          <td>{{$SmDashboardResult[$i]['lss_SlotStatus']}}</td>
                          <td>{{$SmDashboardResult[$i]['lb_LastModifiedDate']}}</td>
                        </tr>
                      @endFor 
